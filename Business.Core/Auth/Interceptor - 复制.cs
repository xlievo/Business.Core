        ...
        public virtual void Intercept(Castle.DynamicProxy.IInvocation invocation)
        {
            ...
            try
            {
                foreach (var item in args.Args)
                {
                    ...
                    if (null != result && 0 >= result.State) { invocation.ReturnValue = Bind.GetReturnValue<Result>(result.State, result.Message, meta); logType = LogType.Exception; return; }
        ...
        */
        ...
        public virtual void Intercept(Castle.DynamicProxy.IInvocation invocation)
        {
            ...
            try
            {
                ...
                {
                    IResult result = null;
                    //var iArgs2 = Bind.GetIArgs(item.IArgs, argsObj, iArgGroup);
                    var value = argsObj[item.Position];
                    var iArgIn = item.HasIArg ? iArgs[item.Position].In : null;
                    ...
                    foreach (var argAttr in item.ArgAttr)
                    {
                        if (item.HasIArg)
                        {
                            var trim = (args.CommandAttr.TrimChar || argAttr.TrimChar) && iArgHasString;
                        ...
                        {
                            var trim = item.HasString && (args.CommandAttr.TrimChar || argAttr.TrimChar) && null != value;
        ...
        static void dsa(object value, dynamic iArgIn, bool iArgHasString, System.Collections.Generic.Dictionary<int, IArg> iArgs, Args item, Attributes.ArgumentAttribute argAttr, bool cmdTrim, object[] argsObj, string methodName, IBusiness business)
        {
            IResult result = null;

            if (item.HasIArg)
            {
                var trim = (cmdTrim || argAttr.TrimChar) && iArgHasString;
                ...
                result = argAttr.Proces(iArgIn, item.IArgOutType, methodName, item.Name, business);
            }
            else
            {
                var trim = item.HasString && (cmdTrim || argAttr.TrimChar) && null != value;
                ...
                result = argAttr.Proces(value, item.Type, methodName, item.Name, business);
            }
