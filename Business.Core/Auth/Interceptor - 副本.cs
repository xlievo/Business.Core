    ...
    public class Interceptor : IInterceptor
    {
        ...
        public virtual async System.Threading.Tasks.Task<dynamic> InterceptAsync(Castle.DynamicProxy.IInvocation invocation)
        {
            ...
            try
            {
                foreach (var item in meta.Args)
                {
                    ...
                    while (NodeState.DAT == first.State)
                    {
                        ...
                        var value2 = item.HasIArg ? iArgIn : value;

                        if (argAttr.Meta.HasProcesIArg)
                        {
                            result = await argAttr.Proces(value2, item.HasIArg ? iArgs[item.Position] : null);
                        }
                        else
                        {
                            result = await argAttr.Proces(value2);
                        }
            ...
            catch (System.Exception ex)
            {
                ...
                invocation.ReturnValue = Bind.GetReturnValue(0, System.Convert.ToString(ex), meta, ResultType);

                if (meta.HasAsync)
                {
                    invocation.ReturnValue = System.Threading.Tasks.Task.FromResult(invocation.ReturnValue);
                }
            ...
            finally
            {
                if (meta.HasAsync)
                {
                    ...
                    if (meta.HasIResult)
                    {
                        invocation.ReturnValue = task.ContinueWith<IResult>(c =>
                        {
                            return Async<IResult>(command, c, ResultType, meta, returnValue, logType, iArgs, argsObj, methodName, this.Logger, watch);
                        });
                    }
                    else
                    {
                        ...
                        {
                            return Async<dynamic>(command, c, ResultType, meta, returnValue, logType, iArgs, argsObj, methodName, this.Logger, watch);
                        });
                    }
                }
                else
        ...
        static dynamic Async2<Type>(CommandAttribute command, System.Type resultType, MetaData meta, dynamic returnValue, LoggerType logType, System.Collections.Generic.Dictionary<int, IArg> iArgs, object[] argsObj, string methodName, System.Action<LoggerData> logger, System.Diagnostics.Stopwatch watch)
        {
            try
            {
                if (meta.HasReturn)
        ...
        async System.Threading.Tasks.Task<IResult> ArgsResult2(string group, System.Collections.Generic.IList<Args> args, string methodName, object currentValue)
        {
            ...
            foreach (var item in args)
            {
                ...
                while (NodeState.DAT == first.State)
                {
                    ...
                    if (argAttr.Meta.HasProcesIArg)
                    {
                        result = await argAttr.Proces(value, (item.HasIArg && null != memberValue) ? (IArg)memberValue : null);
                    }
                    else
                    {
                        result = await argAttr.Proces(value);
                    }
