...
#region Config
        ...
        */
        ...
        static ConcurrentReadOnlyDictionary<string, MetaData> GetMetaData(Configer cfg, Dictionary<int, MethodInfo> methods, dynamic business)
        {
...
#endif
            {
                ...
                var attrs = attributes2.ToReadOnlyDictionary<string, AttributeBase, GroupAttribute>(c => c.GroupKey(), c => c, System.StringComparer.CurrentCultureIgnoreCase);

                var meta = new MetaData(Help.dynamicMethodBuilder.GetDelegate(method), commandGroup, args, childAll, tokens, loggerGroup, method.GetMethodFullName(), name, fullName, cfg.Info.TypeFullName, hasAsync, hasValueTask, hasReturn, hasIResult, hasIResultGeneric, returnType, cfg.ResultTypeDefinition, resultType, GetDefaultValue(args), attrs, methodMeta.Key, cfg.Info.GetCommandGroup(cfg.Info.CommandGroupDefault, name), useTypePosition, GetMethodTypeFullName(fullName, args), attributes2.GetAttr<DocAttribute>(), hasParameters);
    ...
    public class Command
    {
        ...
        public Command(MetaData meta, string key, string group, IBusiness business)
        {
            ...
            this.key = key;
            this.group = group;
            HasHttpFile = Meta.Args.Any(c => c.Group[key].HttpFile) || Meta.Attributes.Values.Any(c => typeof(HttpFileAttribute).IsAssignableFrom(c.Meta.Type));
        ...
        internal void StatisArgs()
        {
            ...
            if (null != ParametersType)
            {
                deserialize = GetDeserialize(Meta.Attributes.Values, Meta.ResultType, Meta.ResultTypeDefinition, ParametersType, group, $"{Meta.CommandGroup.Group[key].OnlyName} Arguments");
            }
        }

        internal static ArgumentAttribute GetDeserialize(IEnumerable<AttributeBase> attributes, System.Type resultType, System.Type resultTypeDefinition, System.Type parametersType, string group, string alias)
        {
